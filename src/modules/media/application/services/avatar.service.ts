...
import type { ConfirmAvatarResponse } from '@/modules/media/application/types/media-upload.type';
...
export class AvatarService {
    ...
    async confirmAvatar(
        ...
        const { user, oldAvatarUrl } =
            await this.authProfileClient.updateAvatar(userId, avatarUrl);
        ...
        try {
            const cleanupResult =
                await this.mediaAssetService.deleteAvatarAsset(
                    userId,
                    oldAssetId,
                );
    ...
    private extractManagedAvatarAssetId(
        ...
        try {
            ...
            if (
                (!isProcessedPath && !isOriginalPath) ||
                parts[3] !== safeUserId
            ) {
                return null;
            }
