...
} from '@/modules/media/application/types/media-upload.type';
import type { AiAssetUploadResponse } from '@/modules/media/application/types/ai-asset-upload.type';
import type { AiAssetUploadDto } from '@/modules/media/presentation/dto/ai-asset-upload.dto';
...
export class MediaAssetService {
    ...
    async uploadAiAsset(dto: AiAssetUploadDto): Promise<AiAssetUploadResponse> {
        ...
        if (!allowedContentTypes.includes(dto.contentType))
            throw new ServiceUnavailableException('Unsupported AI image type');
        ...
        if (body.length > 8 * 1024 * 1024)
            throw new ServiceUnavailableException('AI image is too large');
        await this.s3.send(
    ...
    async downloadInternalAsset(
        ...
        if (!objectKey)
            throw new ServiceUnavailableException(
                'Source media asset is unavailable',
            );
    ...
    private async findProcessedAiAssetKey(
        ownerId: string,
        assetId: string,
