...
export class MediaAssetController {
    ...
    downloadInternalAsset(
        ...
        @Query('purpose')
        purpose: 'product_image' | 'ai_optimization' = 'product_image',
        ...
        if (
            !ownerId ||
            !['product_image', 'ai_optimization'].includes(purpose)
        ) {
            throw new UnauthorizedException('Invalid internal media context');
        }
        return this.mediaAssetService
            .downloadInternalAsset(ownerId, assetId, purpose)
            .then(
                (asset) =>
                    new StreamableFile(asset.body, {
    ...
    cleanupAiAssets(
        ...
        if (!ownerId)
            throw new UnauthorizedException('Missing internal media context');
        return this.mediaAssetService
            .cleanupAiOutputs(ownerId, jobId)
